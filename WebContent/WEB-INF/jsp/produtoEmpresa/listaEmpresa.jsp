...
<h3> Entre com o nome do produto que deseja buscar</h3>
			<form action="<c:url value="/produtoEmpresa/busca3"/>">
...
<display:column title="Ativo" sortable="true"><c:if test="${produtoEmpresa.ativo==1}"> Sim</c:if> <c:if test="${produtoEmpresa.ativo!=1}">Não</c:if> </display:column><display:column title="Editar"><a href="<c:url value="/produtoEmpresa/${produtoEmpresa.id}"/>">Editar</a></display:column>
<display:column title="Remover"><form action="<c:url value="/produtoEmpresa/${produtoEmpresa.id}"/>" method="POST"> <button class="link" name="_method" value="DELETE">Remover</button> </form></display:column>
